...
class AMQPConsumer(object):
    ...
    def __init__(self, host, port, credentials, data_processor,
        ...
        # The `pika.adapters.blocking_connection.BlockingConnection`
        # does not respect the `connection_attempts` parameter.
        # http://pika.readthedocs.io/en/latest/examples/
        # using_urlparameters.html
...
class Publisher(WebSocketClient):
    ...
    def __init__(self, id, kwargs, max_batch_size=100, max_batch_delay=2):
        """Initializes a WebSocket-based Publisher, which is responsible for
        processing new messages consumed by the AMQPConsumer and sending them
        to the specified destination URL in batches.
    ...
    def process_event(self, event):
        """Processes new messages received by the AMQPConsumer and publishes
        batches of data to the desired destination. This method is practically
        the callback function of the AMQPConsumer.
        """
        if 'metric' in event:
            self._process_event__edit_fields(event)

            # We are using '\n' as our delimiter. In our case, this approach
            # makes processing with Heka easier later on.
    ...
    def _process_event__edit_fields(self, event):
        """Edits the fields of a new event and injects into the event dict
        the corresponding cloud provider's resource ID.
        """
        event['time'] = int(datetime.datetime.utcnow().strftime('%s'))
    ...
    def send_data(self, data):
        ...
        log.debug('Sent batch of size %s within %s seconds',
                  self.current_batch_size, time.time() - self.last_push)
        self.write_message(json.dumps(data))


def bootstrap():
    """Main method to initialize the AMQP Middleware by first fetching the
    required configuration options from the systemd env file.
    """

    # Handle boolean expressions as strings for compatibility with
    # Cloudify Blueprints.
